...
def render_prompt(context_id: str, label, cwd: str, job_id: str, prompt: str) -> str:
    ...
    rules. Placeholder names (context_id/label/cwd/job_id/prompt) share no
    substrings, so sequential .replace() calls are unambiguous; `prompt` is
    substituted last so placeholder-like text inside the caller's own prompt is
    never mistaken for a template token.
    """
