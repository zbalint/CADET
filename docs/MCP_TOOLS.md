...
| `skip_permissions` | bool | `false` | Passed through as `agy --dangerously-skip-permissions` when `true`. Default `false` deliberately — see [ARCHITECTURE.md](./ARCHITECTURE.md#validated-agy-cli-behavior) on why leaving `agy`'s own permission soft-deny safety net in place is the safer default for an unattended job. Only set `true` for tasks in a `cwd` you're comfortable giving unrestricted tool access to. |
