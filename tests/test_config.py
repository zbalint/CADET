...
_ENV_VARS = [
    ...
    "CADET_CURSOR_PATH",
    "CADET_CURSOR_DOCKER_IMAGE",
    "CADET_CURSOR_API_KEY",
    "CADET_CURSOR_AUTH_VOLUME",
    "CADET_CURSOR_CONTAINER_MEMORY",
    "CADET_CURSOR_CONTAINER_CPUS",
...
class TestCursorDockerGetters(ConfigTestCase):
    def test_defaults(self):
        self.assertEqual(config.get_cursor_docker_image(), "cadet-cursor:latest")
        self.assertIsNone(config.get_cursor_api_key())
        self.assertEqual(config.get_cursor_auth_volume(), "cadet-cursor-auth")
        self.assertEqual(config.get_cursor_container_memory(), "2g")
        self.assertEqual(config.get_cursor_container_cpus(), "2")
        self.assertEqual(config.get_cursor_container_pids_limit(), 512)
        self.assertEqual(config.get_cursor_stop_grace_s(), 10)

    def test_overrides(self):
        os.environ["CADET_CURSOR_DOCKER_IMAGE"] = "custom-cursor:v2"
        os.environ["CADET_CURSOR_API_KEY"] = "test-key-abc"
        os.environ["CADET_CURSOR_AUTH_VOLUME"] = "custom-cursor-auth"
        os.environ["CADET_CURSOR_CONTAINER_MEMORY"] = "4g"
        os.environ["CADET_CURSOR_CONTAINER_CPUS"] = "4"
        os.environ["CADET_CURSOR_CONTAINER_PIDS_LIMIT"] = "1024"
        os.environ["CADET_CURSOR_STOP_GRACE_S"] = "30"

        self.assertEqual(config.get_cursor_docker_image(), "custom-cursor:v2")
        self.assertEqual(config.get_cursor_api_key(), "test-key-abc")
        self.assertEqual(config.get_cursor_auth_volume(), "custom-cursor-auth")
        self.assertEqual(config.get_cursor_container_memory(), "4g")
        self.assertEqual(config.get_cursor_container_cpus(), "4")
        self.assertEqual(config.get_cursor_container_pids_limit(), 1024)
        self.assertEqual(config.get_cursor_stop_grace_s(), 30)


class TestResolveCursorDockerImage(ConfigTestCase):
    def test_image_found_resolves(self):
        with patch("cadet.config.subprocess.run", return_value=subprocess.CompletedProcess([], 0)) as mock_run:
            self.assertEqual(config.resolve_cursor_docker_image(), "cadet-cursor:latest")
        mock_run.assert_called_once()
        self.assertEqual(mock_run.call_args[0][0], ["docker", "image", "inspect", "cadet-cursor:latest"])

    def test_image_not_found_raises(self):
        with patch("cadet.config.subprocess.run", return_value=subprocess.CompletedProcess([], 1, stderr="no such image")):
            with self.assertRaises(RuntimeError):
                config.resolve_cursor_docker_image()
    ...
    def test_respects_image_env_override(self):
        os.environ["CADET_CURSOR_DOCKER_IMAGE"] = "custom-cursor:v2"
        with patch("cadet.config.subprocess.run", return_value=subprocess.CompletedProcess([], 0)) as mock_run:
            self.assertEqual(config.resolve_cursor_docker_image(), "custom-cursor:v2")
...
class TestProviderGenericResolution(ConfigTestCase):
    ...
    def test_resolve_provider_path_cursor_delegates_to_resolve_cursor_docker_image(self):
        with patch("cadet.config.subprocess.run", return_value=subprocess.CompletedProcess([], 0)):
