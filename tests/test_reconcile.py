...
class TestReconcileOnStartup(ReconcileTestCase):
    ...
    async def test_running_cursor_job_stops_container_regardless_of_pid_liveness(self):
        # Mirrors test_running_codex_job_stops_container_regardless_of_pid_liveness
        # — cursor is containerized as of Phase 4, same "docker stop
        # unconditionally, never PID-liveness-check" rule as agy/codex.
        self._insert("job-running-cursor", status="pending", provider="cursor")
        job_store.mark_running("job-running-cursor", pid=9999, started_at="t1", db_path=self.db_path)
        ...
        mock_stop.assert_called_once_with("cadet-cursor-job-running-cursor", ANY)
        mock_kill.assert_not_called()
        mock_alive.assert_not_called()
